...
class ContentHeader : public XIAGenericExtHeader { public:
    ...
    uint8_t opcode()
    {
        if (!exists(OPCODE))
            return 0;
        return *(const uint8_t*)_map[OPCODE].data();
    };
    ...
    uint16_t offset()
    {
        if (!exists(OFFSET))
            return 0;
        return *(const uint16_t*)_map[OFFSET].data();
    }
    uint32_t chunk_offset()
    {
        if (!exists(CHUNK_OFFSET))
            return 0;
        return *(const uint32_t*)_map[CHUNK_OFFSET].data();
    }
    uint16_t length()
    {
        if (!exists(LENGTH))
            return 0;
        return *(const uint16_t*)_map[LENGTH].data();
    }
    uint32_t chunk_length()
    {
        if (!exists(CHUNK_LENGTH))
            return 0;
        return *(const uint32_t*)_map[CHUNK_LENGTH].data();
    }
...
class ContentHeaderEncap : public XIAGenericExtHeaderEncap { public:
    ...
    static ContentHeaderEncap* MakeRequestHeader()
    {
        return new ContentHeaderEncap(ContentHeader::OP_REQUEST,0,0);
    }
    static ContentHeaderEncap* MakeRPTRequestHeader()
    {
        return new ContentHeaderEncap(ContentHeader::OP_REDUNDANT_REQUEST,0,0);
    }
