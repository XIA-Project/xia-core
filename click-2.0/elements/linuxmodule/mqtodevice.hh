#ifndef CLICK_MQTODEVICE_HH
#define CLICK_MQTODEVICE_HH

/*
=c

MQToDevice(DEVNAME, QUEUE [, BURST, I<KEYWORDS>])
...
This manual page describes the Linux kernel module version of the MQToDevice
element using multi-queues feature. For the user-level element, read the MQToDevice.u manual page.
...
case MQToDevice searches for a device with that address.
...
Packets must have a link header. For Ethernet, MQToDevice makes sure every
...
the router is initialized, then MQToDevice will report a warning (rather than an
error). Later, while the router is running, if a device named DEVNAME appears,
MQToDevice will seamlessly begin sending packets to it. Default is false.
...
packets. Linux may cause the device to be busy when a MQToDevice wants to send a
packet. Click is not clever enough to re-queue such packets, and discards
them.

In Linux 2.2, whether or not the device is running in polling mode, MQToDevice
...
Packets sent via MQToDevice will not be received by any packet sniffers on the
machine. Use Tee and ToHostSniffers to send packets to sniffers explicitly.

=h count read-only

Returns the number of packets MQToDevice has pulled.

=h calls read-only

Returns a summary of MQToDevice statistics.

=h drops read-only

Returns the number of packets MQToDevice has dropped.  MQToDevice will drop
...
=a FromDevice, PollDevice, FromHost, ToHost, MQToDevice.u, Tee, ToHostSniffers
...
class MQToDevice : public AnyTaskDevice { public:
