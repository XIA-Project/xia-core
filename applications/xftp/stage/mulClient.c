...
void *mainLoop(void * ifaddr)
{
	...
	for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {  
        if (ifa->ifa_addr == NULL)  
            continue;  
        printf("interface: %s \n", ifa->ifa_name); 
		if(strcmp(ifa->ifa_name, "iface0")==0 && iface == 0) break;//"wlp6s0"
		if(strcmp(ifa->ifa_name, "iface1")==0 && iface == 1) break;//"wlx60a44ceca928"
    } 
	int ssock;
	int count = 0;
	int printcount = 1;
...
int main(int argc, char **argv)
{
	...
	*if1 = 0;
	*if2 = 1;
	pthread_create(&clients[0], NULL, watch_network, (void *)if1);
	pthread_create(&clients[1], NULL, watch_network, (void *)if2);
	for (int i = 0; i < 2; i++) {
