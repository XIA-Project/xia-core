    ...
    return {
        ...
        link: function (scope, elem, attrs) {
            ...
            scope.$watch('bufferData', function(v) {
                ...
                if (!chart1 && attrs.ngModel == "bufferData") {
                    chart1 = $.plot(elem, v , options1);
                    elem.show();
                }
                else if (chart1){
                    chart1.setData(v);
            ...
            // If the data changes somehow, update it in the chart
            scope.$watch('bitrateData', function(v) {
                if (v === null || v === undefined) {
                    return;
                }

                if (!chart2 && attrs.ngModel == "bitrateData") {
                    chart2 = $.plot(elem, v , options2);
                    elem.show();
                }
                else if (chart2){
                    chart2.setData(v);
...
app.controller('DashController', function($scope, Sources, Notes, Contributors, PlayerLibraries, ShowcaseLibraries) {
    ...
    $scope.totalBitrate = 0;
    $scope.numBitrates = 0;
    $scope.averageBitrate = 0;
    
    $scope.totalBufferLength = 0;
    $scope.numBufferLength = 0;
    $scope.averageBufferLength = 0;
    ...
    function metricChanged(e) {
        ...
        if (e.mediaType == "video") {
            ...
            if (metrics) {
                ...
                $scope.totalBitrate += $scope.videoBitrate;
                $scope.numBitrates += 1;
                $scope.averageBitrate = ($scope.totalBitrate/$scope.numBitrates).toFixed(3);
                ...
                $scope.totalBufferLength += parseFloat(metrics.bufferLengthValue);
                $scope.numBufferLength += 1;
                $scope.averageBufferLength = ($scope.totalBufferLength/$scope.numBufferLength).toFixed(3);
                ...
                point2 = [parseFloat(video.currentTime), Math.round(parseFloat(metrics.bandwidthValue))];

                bufferLengthSeries.push(point);
                bitrateSeries.push(point2);

                if (bufferLengthSeries.length > maxGraphPoints) {
                    bufferLengthSeries.splice(0, 1);
                }
                if (bitrateSeries.length > maxGraphPoints) {
                    bitrateSeries.splice(0, 1);
                }
            }
        }
