...
def update_stockfeed(stock):
    ...
    updata = "<span class=\"clstsu\">%s %.2f  +%.2f</span> "
    downdata = "<span class=\"clstsd\">%s %.2f %.2f</span> "
    stable = "<span class=\"clstst\">%s %.2f %.2f</span> "
...
while(True):
	...
	print "request received response len %d %s" % (len(response), response)
