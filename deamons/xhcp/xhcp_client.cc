...
int main(int argc, char *argv[]) {
	...
	/* Main operation:
		1. receive myAD & gateway router HID information from XHCP server
		2. update AD table: 
			(myAD, 4, -, -)
			(default, 0, gwRHID, -)
		3. update HID table:
	...
	*/
	...
	while(!(quit_flag & 2)) {
		...
		// validate pkt
		if (strlen(self_dag) <= 0 || strlen(gw_dag) <= 0) {
			continue;
		}
		if (adv_selfdag != NULL && adv_gwdag != NULL) {
			if (!strcmp(adv_selfdag, self_dag) && !strcmp(adv_gwdag, gw_dag)) {
				continue;
			}
		}
