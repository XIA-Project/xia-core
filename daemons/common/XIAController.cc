...
using namespace std;
#include "XIAController.hh"

int XIAController::connect(std::string clickHost, unsigned short controlPort)
{
	struct hostent *h;
...
int XIAController::version(std::string &ver)
{
	if (!connected())
		return XR_NOT_CONNECTED;
...
int XIAController::getRoutes(std::string xidtype, std::vector<XIARouteEntry> &xrt)
{
	std::string result;
	vector<string> lines;
	int n = 0;
...
std::string XIAController::itoa(signed i)
{
	std::string s;
	std::stringstream ss;
...
int XIAController::updateRoute(string cmd, const std::string &xid, int port, const std::string &next, unsigned long flags)
{
	string xidtype;
...
int XIAController::addRoute(const std::string &xid, int port, const std::string &next, unsigned long flags)
{
	return updateRoute("add4", xid, port, next, flags);
}

int XIAController::setRoute(const std::string &xid, int port, const std::string &next, unsigned long flags)
{
	return updateRoute("set4", xid, port, next, flags);
}

int XIAController::delRoute(const std::string &xid)
{
	string next = "";
	return updateRoute("remove", xid, 0, next, 0);
}

const char *XIAController::cserror()
{
	switch(_cserr) {
