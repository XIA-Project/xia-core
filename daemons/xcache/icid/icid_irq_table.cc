...
ICIDIRQTable* ICIDIRQTable::_instance = 0;

ICIDIRQTable::ICIDIRQTable()
{
	_instance = 0;
}

ICIDIRQTable* ICIDIRQTable::get_table()
{
	if(_instance == 0) {
		_instance = new ICIDIRQTable;
	}
	return _instance;
}
...
bool ICIDIRQTable::has_entry(std::string cid)
{
...
bool ICIDIRQTable::add_fetch_request(std::string cid, std::string requestor)
{
...
RequestorList ICIDIRQTable::requestors(std::string cid)
{
