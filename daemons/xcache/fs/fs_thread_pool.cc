 ...
 */
void FSThreadPool::work()
{
	std::cout << std::this_thread::get_id() << ": Started" << std::endl;
	while(!_stop) {
		FSWorkRequestPtr work = fetch_work();
		work->process();
		// At this point the unique_ptr work goes out of scope
		// and FSWorkRequest destructor should get called
	}
}
 ...
 */
int FSThreadPool::queue_work(FSWorkRequestPtr work)
{
 ...
 * to complete.
 */
FSWorkRequestPtr FSThreadPool::fetch_work()
{
	...
	FSWorkRequestPtr request = std::move(_work_queue.front());
	_work_queue.pop();
