...
int ROTParser::verifyROT(const ROT& rot) {
    ...
    while (ptr2!=NULL) {
        uint64_t aid = strtoull(ptr2->FirstChildElement("AID")->GetText(), 
            NULL, 10);
        // get sig length
        ...
        memcpy(signatures.find(aid)->second, 
            ptr2->FirstChildElement("sign")->GetText(), sigLen);
        ptr2 = ptr2->NextSiblingElement();
    }
    ...
    for (itr = signatures.begin(); itr!=signatures.end(); ++itr) {
        ...
        size_t len = 512;
        memset(output, 0, 512);
        ret = base64_decode(output, &len, 
            itr->second, sigLengths.find(itr->first)->second);
        ...
        while (cur != NULL) {
            ...
            if ((ret = rsa_pkcs1_verify(pk_rsa(cur->pk), NULL, NULL, 
                 RSA_PUBLIC, POLARSSL_MD_SHA1, 20, sha1Hash, output)) != 0) 
            {
