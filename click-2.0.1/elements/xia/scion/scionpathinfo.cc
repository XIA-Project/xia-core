/*
 * scionpathinfo.{cc,hh} -- SCION Path Infobase Element
 * Chang Lan
 *
 * TODO: 
 *  1. Query
...
{   
    for (std::map<ADIdentifier, std::vector<halfPath*> >::iterator it = m_vDownpaths.begin(); it != m_vDownpaths.end(); ++it) {
...
{   
	for (std::map<ADIdentifier, std::vector<halfPath*> >::iterator it = m_vUppaths.begin(); it != m_vUppaths.end(); ++it) {
        for (std::vector<halfPath*>::iterator jt = it->second.begin(); jt != it->second.end(); ++jt) {
            if ((*jt)->path_marking)
                delete [] (*jt)->path_marking;
			delete *jt;
...
SCIONPathInfo::parse(const void *p, int type)
{
	...
	uint8_t srcLen = SCIONPacketHeader::getSrcLen((uint8_t *)p);
	uint8_t dstLen = SCIONPacketHeader::getDstLen((uint8_t *)p);
    //extracting information from packet header

	//SL: Caution: downpath includes OF to TDC
	uint8_t hdrLen = SCIONPacketHeader::getHdrLen((uint8_t *)p);
    ...
    if (type) {
		...
		ADIdentifier endpoint = received_path_ptr->path[received_path_ptr->path.size() - 1];
        ADIdentifier core = received_path_ptr->path[0];
	...
	//merge these together after changing the message format...
		...
		uint16_t info_length = SCIONPacketHeader::getTotalLen((uint8_t *)p) - hdrLen - PATH_INFO_SIZE;
		#ifdef _SL_DEBUG_GW
...
*/
bool 
SCIONPathInfo::get_path(ADIdentifier src, ADIdentifier dst, fullPath& endpath) 
{
    std::map<ADIdentifier, std::vector<halfPath*> >::iterator it_down = m_vDownpaths.find(dst);
    std::map<ADIdentifier, std::vector<halfPath*> >::iterator it_up = m_vUppaths.find(src);
...
*/
bool
SCIONPathInfo::initAdTable(ADIdentifier src, ADIdentifier dst) {
    ...
    // UP-PATHS
    for (std::vector<halfPath*>::iterator it_up_path = it_up->second.begin(); it_up_path != it_up->second.end(); ++it_up_path) { //for each up-path
        addUpPath(*(*it_up_path));     
    }

    // DOWN-PATHS
    for (std::vector<halfPath*>::iterator it_down_path = it_down->second.begin(); it_down_path != it_down->second.end(); ++it_down_path) { //for each down-path
...
SCIONPathInfo::getShortestPath(DownPathADTable& down_path_ad_table, fullPath& endpath) {
  click_chatter("get shortest pat");
  return false;
    ...
    for (std::map<ADIdentifier, DownPathAD>::iterator it_down_ad = down_path_ad_table.begin(); 
		it_down_ad != down_path_ad_table.end(); ++it_down_ad) {
        //1. search cross-over AD (i.e., common AD of up & down paths) on up-paths
        std::map<ADIdentifier, UpPathAD>::iterator it_up_ad_xover = m_uppathAdTable.find(it_down_ad->first);
        ...
        for (std::vector<uint64_t>::iterator it_peer = it_down_ad->second.peer_ad.begin(); 
            ...
            std::map<ADIdentifier, UpPathAD>::iterator it_up_ad_peer = m_uppathAdTable.find(*it_peer);
    ...
    if (end2end_path_type != PATH_TYPE_TDC) { 
        ...
        std::map<ADIdentifier, UpPathAD>::iterator itr_up_ad = m_uppathAdTable.find(upAD);
        std::map<ADIdentifier, DownPathAD>::iterator itr_down_ad = down_path_ad_table.find(dwAD);
...
*/
void
SCIONPathInfo::constructFullPath(ADIdentifier upAD, ADIdentifier dwAD, halfPath& upPath, halfPath& downPath, 
uint16_t end2end_up_hops, uint16_t end2end_down_hops, uint8_t end2end_path_type, fullPath& endpath) {
	...
	if (end2end_path_type == PATH_TYPE_XOVR) { //cross-over
		...
		*pSO = *pTS;
		...
		//Up-path
		#ifdef _SL_DEBUG_GW
		printf("\nUp path in reverse order (from TDC) upPath.hops = %d, downPath.hops = %d, upHop=%d, dwHop=%d\n",
		...
		*pSO = *pTS;
		...
		pSO->info = 0xc0; //indicates TDC path
    ...
    if (end2end_path_type == PATH_TYPE_PEER) { //peer
		#ifdef _SL_DEBUG_GW
		printf("Construct a peering path\n");
		#endif
		...
		*pSO = *pTS;
		...
		opaqueField * pOF;
		#ifdef _SL_DEBUG_GW
		printf("\nUp path in reverse order (from TDC)...\n");
		#endif
		...
		*pSO = *pTS;
		...
		pSO->info = 0xf0; //indicates peer path
...
SCIONPathInfo::storeOpaqueField(HostAddr &addr, fullPath &of) {
	...
	if((itr = m_inOF.find(addr)) == m_inOF.end()) {
		...
		//if the size of the list exceeds the threashold, remove the oldest one
