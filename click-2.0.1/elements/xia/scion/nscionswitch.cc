...
#include"nscionswitch.hh"


CLICK_DECLS
int NSCIONSwitch::configure(Vector<String> &conf, ErrorHandler *errh){
...
int NSCIONSwitch::initialize(ErrorHandler* errh){
   initVariables();
...
void NSCIONSwitch::run_timer(Timer* timer){
   ...
   for(int i=0;i<numPort;i++){
        ...
        SCIONPacketHeader::setType(buf, AID_REQ);
		SCIONPacketHeader::setSrcAddr(buf, s_addr);
		SCIONPacketHeader::setTotalLen(buf, COMMON_HEADER_SIZE+MAX_HOST_ADDR_SIZE);
...
*/
void NSCIONSwitch::push(int port, Packet* pkt){
    
    uint8_t type = SCIONPacketHeader::getType((uint8_t*)pkt->data());
    ...
    uint16_t o_interface = SCIONPacketHeader::getOutgoingInterface(packet);
	...
	if(type == DATA) {
        ...
        if (itr1!=ifid2addr.end() || !o_interface) {
			HostAddr target;
			if(!o_interface){
				target = SCIONPacketHeader::getDstAddr(packet);
	...
	//2. OW: interface -> destination router address -> port
		...
		if(type > TO_LOCAL_ADDR) {
            ...
            HostAddr addr = SCIONPacketHeader::getDstAddr(packet);
    	    int port;
			//SLT: Map
		...
		//very infrequent: only performed when AID is requested during the bootstrapping
    		...
    		HostAddr addr = SCIONPacketHeader::getSrcAddr(packet);
			//SLT:
...
void NSCIONSwitch::addToPendingTable(uint8_t * packet, uint8_t & type, uint16_t & len, HostAddr & addr) {
...
void NSCIONSwitch::parseTopology(){
    TopoParser parser;
...
void NSCIONSwitch::initVariables(){
    Config config;
...
void NSCIONSwitch::sendPacket(uint8_t* data, uint16_t dataLength, int port ){
    ...
    uint32_t ts = SCIONPacketHeader::getTimestamp(data);
