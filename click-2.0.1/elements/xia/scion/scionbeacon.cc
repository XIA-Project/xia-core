...
*/
uint32_t SCIONBeaconLib::createMAC(uint32_t ts, uint8_t exp, uint16_t ingress, uint16_t egress, uint64_t prev_of,
aes_context* ctx){
...
/*
    ...
    - check if the given MAC is valid by comparing the target MAC with the newly
      generated MAC.
*/
int SCIONBeaconLib::verifyMAC(uint32_t ts, uint8_t exp, uint16_t ingress, uint16_t egress, uint64_t prev_of, uint32_t mac,
aes_context* ctx){
...
uint8_t SCIONBeaconLib::addLink(uint8_t* pkt,uint16_t ingress,uint16_t egress,
    ...
    uint16_t totalLen = SCIONPacketHeader::getTotalLen(pkt);
	...
	#ifdef _SL_DEBUG_BS
	printf("Adding a link: totalLen=%d, hops=%d, hdrLen=%d, ts = %d\n", totalLen, hops, hdrLen,ts);
	#endif
    ...
    if(type){
        ...
        SCIONPacketHeader::addTotalLen(pkt, PCB_MARKING_SIZE);
        addNumHops(pkt ,1);
    }else{
        ...
        SCIONPacketHeader::addTotalLen(pkt, PEER_MARKING_SIZE);
    }
...
ofType, uint8_t exp, uint16_t bwAlloc){
    ...
    if(type){
        ...
        memcpy(pkt+totalLen, (uint8_t*)&newMarking, PCB_MARKING_SIZE);
        SCIONPacketHeader::addPacketLen(pkt, PCB_MARKING_SIZE);
        SCIONPacketHeader::addNumOF(pkt ,1); 
...
uint8_t SCIONBeaconLib::signPacket(uint8_t* pkt, int sigLen, uint64_t &next_aid, rsa_context* ctx){
    ...
    SCIONPacketHeader::addTotalLen(pkt,sigLen);
...
void SCIONBeaconLib::printPCB(uint8_t* pkt){
    uint16_t totalLength = SCIONPacketHeader::getTotalLen(pkt);
    uint8_t hops = SCIONPacketHeader::getNumOF(pkt);
    printf("====== PCB PRINT =======\n");
    printf("%d   %d\n",totalLength,hops);
...
void SCIONBeaconLib::addNumHops(uint8_t* pkt, uint8_t hops){
    ...
    //uint8_t hdrLen = COMMON_HEADER_SIZE+SCIONPacketHeader::getSrcLen(pkt)+SCIONPacketHeader::getDstLen(pkt);
    uint8_t hdrLen = SCIONPacketHeader::getHdrLen(pkt);
