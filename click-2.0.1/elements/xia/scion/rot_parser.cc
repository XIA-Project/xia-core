...
int ROTParser::parse(ROT &rot){
	...
	strptime(ptr->FirstChildElement("issueDate")->GetText(), "%b %d %H:%M:%S %Y", &(rot.issuedate));
	strptime(ptr->FirstChildElement("expireDate")->GetText(), "%b %d %H:%M:%S %Y %Z", &rot.expiredate);
...
int ROTParser::verifyROT(const ROT& rot){
	...
	if(signatures.size()!=rot.numCoreAD){
		printf("ROT Verification Failed: # of signatures does not equal to # of certificates.\n");
	...
	for(itr=signatures.begin();itr!=signatures.end();itr++){
		...
		while( cur != NULL )
		{
			if( ( ret = rsa_pkcs1_verify( &cur->rsa, RSA_PUBLIC, SIG_RSA_SHA1, 20, sha1Hash, output ) ) != 0 )
			{
