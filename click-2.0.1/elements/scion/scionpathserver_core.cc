...
bool SCIONPathServerCore::run_task(Task* task){
    ...
    while((inPacket=input(PORT_TO_SWITCH).pull())){
        ...
        /*PATH_REG: path registration packet from local pcb server*/
			...
			printf("\t buf: %s\n",buf);
			#endif
			scionPrinter->printLog(IH,type,"PS(%llu:%llu),RECEIVED PATH REGISTRATION: %s\n"
			,m_uAdAid, m_uAid,buf);
        ...
        /*PATH_REQ: path request packet form the local path server*/
            ...
            for(itr=paths.begin();itr!=paths.end();itr++){
                ...
                if(itr->first == target){
					...
					#ifdef _SL_DEBUG_PS
					printf("TDC PS: AD%d -- #of Registered Paths: %d\n",target,itr->second.size());
					#endif
...
*/
void SCIONPathServerCore::sendPacket(uint8_t* data, uint16_t data_length, int port, int fwd_type){
