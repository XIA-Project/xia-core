    ...
    while((inPacket=input(PORT_TO_SWITCH).pull())){
		...
		case PATH_REP:{
			...
			for(itr = requesters.first; itr != requesters.second; itr++) {
				...
				#endif
				scionPrinter->printLog(IH,type,"PS (%llu:%llu): Sending Downpath to Client: %llu\n", 
		...
		case PATH_REQ_LOCAL:{
			...
			scionPrinter->printLog(IH,type,"PS (%llu:%llu): Request Downpath from Client: %llu\n", 
...
void SCIONPathServer::sendPacket(uint8_t* data, uint16_t data_length, int port, int fwd_type){
