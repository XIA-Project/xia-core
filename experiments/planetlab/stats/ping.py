...
processes = [Popen(shlex.split(ping + '1 -w 1 ' + node), stdout=PIPE) for node in nodes]
outs = [process.communicate() for process in processes]
rcs = [process.wait() for process in processes]

processes = [Popen(shlex.split(ping + '%s -w 5 ' % count + node), stdout=PIPE) for node in nodes]
outs = [process.communicate() for process in processes]
rcs = [process.wait() for process in processes]
outs = zip(outs, rcs)
