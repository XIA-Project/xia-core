...
**
*/
char *findRoot(char *path, unsigned size) {
	char *pos;

	if (!getcwd(path, size))
		return NULL;

	if ((pos= strstr(path, SOURCE_DIR))) {
...
char *hostsLookup(const char *name) {
	...
	// get the xia tree root path
	if (!findRoot(path, BUF_SIZE))
		return NULL;

	FILE *hostsfp = fopen(strcat(path, ETC_HOSTS), "r");
	int answer_found = 0;
	if (hostsfp != NULL) {
