...
def ifconfig(device)
  ...
  result.each_line do |line|
    if (line=~/RX bytes:/)
       stats[1] = line.split(':')[1].to_i()
       stats[2] = line.split(':')[2].to_i()
    end 
    if (line=~/RX packets:/)
       stats[3] = line.split(':')[1].to_i()
    end 
    if (line=~/TX packets:/)
       stats[4] = line.split(':')[1].to_i()
    end 
  end
  return stats
end
...
if __FILE__== $0
  ...
  while (timediff+ report_interval/2 < time_interval)
     ...
     puts  " RX " + diff(last[0][1], prev[0][1]).to_s() +" TX "  + diff(last[0][2], prev[0][2]).to_s()
  ...
  puts "RX #{"%.2f" % (rx/timediff*8/1000000)} Mbps TX #{"%.2f" % (tx/timediff*8/1000000)} Mbps"
  puts "RX  #{"%.2f" % (rxp/timediff)} pps TX #{"%.2f" %(txp/timediff)} pps"
  puts "RX  #{rxp} packets TX #{txp} packets"
