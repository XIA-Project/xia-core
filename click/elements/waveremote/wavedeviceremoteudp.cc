/**
 * wavedeviceremoteudp.{cc,hh} -- wave device remote interface
...
#include <sys/socket.h> /* connect(), sendto(), recvfrom() */
...
WaveDeviceRemoteUdp::WaveDeviceRemoteUdp() : _task(this),
                                             _wq(0),
                                             _rcvrTid(0){
    _pipeFd[0] = 0;
    _pipeFd[1] = 0;
...
WaveDeviceRemoteUdp::~WaveDeviceRemoteUdp(){
}


int WaveDeviceRemoteUdp::configure(Vector<String> &conf, ErrorHandler *errh){
 ...
 */
int WaveDeviceRemoteUdp::initialize(ErrorHandler *errh){

    struct addrinfo hints, *servinfo = NULL, *p;
    _sockaddrLen = sizeof(struct sockaddr);
...
strerror=%s", declaration().c_str(), strerror(errno));
            ...
            // copy information
            _remoteSockaddr = (sockaddr *) malloc(p->ai_addrlen);
            memcpy(_remoteSockaddr, p->ai_addr, p->ai_addrlen);

            uint8_t connectCode = WAVESERVER_CON_CODE;
            if (sendto(_remoteSockFd, &connectCode, 1 /*len*/, 0 /*flags*/, \
                _remoteSockaddr, _sockaddrLen) < 0) {

                errh->error("%s, initialize(): sendto() error trying to send \
connect code, strerror=%s", declaration().c_str(), strerror(errno));
...
void WaveDeviceRemoteUdp::add_handlers(){
 ...
 */
void WaveDeviceRemoteUdp::cleanup(CleanupStage){
...
strerror=%s", declaration().c_str(), strerror(errno));
    ...
    if (_isConnected){
        ...
        uint8_t disconnectCode = WAVESERVER_DISC_CODE;
        if (sendto(_remoteSockFd, &disconnectCode, 1 /*len*/, 0 /*flags*/, \
            _remoteSockaddr, _sockaddrLen) < 0) {

        errh->error("%s, cleanup(): sendto() error trying to send \
disconnect code, strerror=%s", declaration().c_str(), strerror(errno));
 ...
 */
void WaveDeviceRemoteUdp::push(int, Packet *p) {
 ...
 */
bool WaveDeviceRemoteUdp::run_task(Task*){
 ...
 */
int WaveDeviceRemoteUdp::write_packet(Packet *p, ErrorHandler *errh){
    ...
    while (not done){
        ...
        errno = 0;
        if (sendto(_remoteSockFd, pktBuf, pktLen, 0 /*flags*/, \
            _remoteSockaddr, _sockaddrLen) < 0){ // error
 ...
 */
void* WaveDeviceRemoteUdp::receiver_thread(void *arg){
    ...
    WaveDeviceRemoteUdp* wdrInst = (WaveDeviceRemoteUdp*) arg;
    ...
    struct sockaddr *remoteSockaddr = wdrInst->get_remoteSockaddr();
    socklen_t sockaddrLen = wdrInst->get_sockaddrLen();
    ...
    while (true){
        ...
        if ((nrcvd = recvfrom(remoteSockFd, rcvBuf, bufLen, 0 /*flags*/, \
            remoteSockaddr, &sockaddrLen)) > 0){

            // got something to work with

#ifdef DEBUG
            errh->debug("%s, rcvd a packet w/ %d bytes", 
...
pthread_mutex_lock(pipeMutex), strerror=%s", 
                wdrInst->declaration().c_str(), strerror(errno));
            }
 ...
 */
void WaveDeviceRemoteUdp::selected(int fd, int /*mask*/){
...
bool WaveDeviceRemoteUdp::isConnected() const{
    return _isConnected;
}


int WaveDeviceRemoteUdp::get_bufLen() const{
    return _bufLen;
}


int WaveDeviceRemoteUdp::get_remoteSockFd() const{
    return _remoteSockFd;
}


int WaveDeviceRemoteUdp::get_pipefd(const bool write) const{
...
struct sockaddr *WaveDeviceRemoteUdp::get_remoteSockaddr() const{
    return _remoteSockaddr;
}


socklen_t WaveDeviceRemoteUdp::get_sockaddrLen() const{
    return _sockaddrLen;
}


pthread_mutex_t* WaveDeviceRemoteUdp::get_pipeMutex(){
    return &_pipeMutex;
}
...
CLICK_ENDDECLS
EXPORT_ELEMENT(WaveDeviceRemoteUdp)
