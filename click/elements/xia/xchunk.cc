...
XChunk::XChunk(XTRANSPORT *transport, const unsigned short port)
	: sock(transport, port, TransportHeader::XSOCK_CHUNK), _q_recv(this), _q_usr_input(this)
{
 ...
 *  order.
 */
CTCPQueue::CTCPQueue(XChunk *con)
{
	_con = con ;
...
CTCPQueue::pull_front()
{
	...
	// assigns _q_first->nxt	_q_first = _q_first->nxt;
