...
XIAReassembler holds them until it has enough fragments to recreate a complete
...
seconds, the fragments are generally dropped. If XIAReassembler has two
outputs, however, a single packet containing all the received fragments at
their proper offsets is pushed onto output 1.

XIAReassembler's memory usage is bounded. When memory consumption rises above
HIMEM bytes, XIAReassembler throws away old fragments until memory consumption
...
XIAReassembler destroys its input packets' "next packet" annotations.

=a IPFragmenter */

class XIAReassembler : public Element { public:

    XIAReassembler() CLICK_COLD;
    ~XIAReassembler() CLICK_COLD;

    const char *class_name() const	{ return "XIAReassembler"; }
...
inline int
XIAReassembler::bucketno(const click_ip *h)
{
    return (h->ip_id % NMAP);
}

inline bool
XIAReassembler::same_segment(const click_ip *h, const click_ip *h2)
{
